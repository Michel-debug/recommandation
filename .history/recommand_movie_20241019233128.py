...
class RecommandMovie:
    ...
    def predict_ratings_for_item(self,user_id,k=10):
        user_ratings = self.rating_matrix.loc[user_id+1]
        rated_movies = user_ratings[user_ratings.notna()].index
        unrated_movies = user_ratings[user_ratings.isna()].index
        if len(unrated_movies) == 0:
            print(f"User{user_id} has rated all the movies")
            return
        similar_items_matrix = self.item_similarity.loc[unrated_movies, rated_movies]
        def
    ...
    def recommend_movies_user_itemrc(self,top_n=1,k=10):
        ...
        for index,user_id in enumerate(user_ids):
             predicted_ratings = self.predict_ratings_for_item(user_id+1,k)
        #     user_ratings = self.rating_matrix.loc[user_id+1]
        #     unrated_movies = user_ratings[user_ratings.isna()].index
        #     predictions = {}
...
if __name__ == '__main__':
    ...
    RecommandMovie.printResult(RecommandMovie(data_path,chooce_model=choose_model[0]).recommend_movies_user_itemrc(top_n=1))