...
class RecommandMovie:
    ...
    @timeit
    @printLog
    def recommend_movies_user_userrc(self,user_id, top_n=1, k=10):
        result = []
        user_ids = list(self.rating_data.index)[:50]
        # 预测用户对未评分电影的评分
        for user_id in user_ids:
            predicted_ratings = self.predict_ratings_for_user(user_id, k)
        # 按照预测评分排序，推荐前 top_n 个电影
            recommendations = predicted_ratings.sort_values(ascending=False).head(top_n)
            result.append((user_id,recommendations))
        return result
...
if __name__ == '__main__':
    ...
    print(RecommandMovie(data_path).recommend_movies_user_userrc(user_id))