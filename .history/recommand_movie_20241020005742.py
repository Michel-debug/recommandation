...
class RecommandMovie:
    ...
    def recommend_movies_user_userrc(self,top_n=1, k=10):
        ...
        for p in processes:
            p.join()
        RecommandMovie.printResult(list(result))
        
    
    def process_user_item(self,user_id,top_n,k):
        ...
        return [user_id, recommended_movies[0][0], round(recommended_movies[0][1], 1)]
    ...
    def recommend_movies_user_itemrc(self, top_n=1, k=10):
        ...
        # 创建部分函数，固定 top_n 和 k 参数
        process_user = partial(self.process_user_item, top_n=top_n, k=k)
