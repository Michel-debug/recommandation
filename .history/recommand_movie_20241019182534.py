...
class RecommandMovie:
    ...
    def evaluate_model(self, test_size=0.2, k=10):
        ...
        user_similarity = self.cosine_similarity_user()
        
        # 3. 预测测试集中的评分
        test_data = test_data.copy()
        test_data['predicted_rating'] = test_data.apply(
            lambda row: self.predict_rating(row['user_id'], row['movie_id'],self.rating_matrix,k), axis=1
        )
...
if __name__ == '__main__':
    ...
    RecommandMovie(data_path).evaluate_model()