    ...
    def recommend_movies_user_itemrc(self,top_n=1,k=10):
        ...
        for user_id in user_ids:
            ...
            for movie_id in unrated_movies:
                predicted_rating = self.predict_rating_item_based(user_id+1, movie_id+1, self.rating_matrix, self.item_similarity, k)
                predictions[movie_id] = predicted_rating
            # 按照预测评分排序，推荐前 top_n 个电影
            recommended_movies = sorted(predictions.items(), key=lambda x: x[1], reverse=True)[:top_n]
            result.append([user_id,list(recommended_movies.keys())[0],round(list(recommended_movies.values())[0],1)])
            print(result[0])
        return result
