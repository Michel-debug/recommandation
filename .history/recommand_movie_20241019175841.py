...
class RecommandMovie:
    ...
    # 检查用户和电影是否在矩阵中
    if user_id not in self.user_similarity.index or movie_id not in self.rating_matrix.columns:
        # 返回全局平均评分
        global_mean = self.rating_matrix.values.mean()
        return global_mean
    ...
    if similar_users.empty:
        # 返回用户平均评分或全局平均评分
        user_mean = self.rating_matrix.loc[user_id].mean()
    ...
    # 获取这些用户对目标电影的评分
    similar_users_ratings = self.rating_matrix.loc[similar_users.index, movie_id]
    ...
    # 将预测评分限制在评分范围内（例如1到5）
    predicted_rating = max(min(predicted_rating, 5), 1)
