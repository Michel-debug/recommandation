...
class RecommandMovie:
    ...
    def cosine_similarity_item(self):
        ...
        dot_projuct = np.dot(norm_matrix_T,norm_matrix)
        norm =  np.linalg.norm(dot_projuct,axis=1)
        epsilon = 1e-8
    ...
    def predict_rating(self, user_id, movie_id, rating_matrix, user_similarity, k=10):
        ...
        # 去除未评分的用户
        mask = similar_users_ratings.notnull()
        if mask.sum() == 0:
            # 如果没有相似用户对该物品评分，返回用户平均评分
            return user_mean

        # 获取相似用户的平均评分
        similar_users_mean = rating_matrix.loc[similar_users.index].mean(axis=1)
        
        # 计算加权偏差
        adjusted_ratings = similar_users_ratings[mask] - similar_users_mean[mask]
        weights = similar_users[mask]
        numerator = np.dot(weights, adjusted_ratings)
...
if __name__ == '__main__':
    ...
    RecommandMovie(data_path).evaluate_model(k=2000)